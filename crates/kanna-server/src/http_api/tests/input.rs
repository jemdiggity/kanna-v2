...
#[tokio::test]
async fn send_task_input_route_resolves_running_stage_run_session() {
    use kanna_daemon::protocol::{Command as DaemonCommand, Event as DaemonEvent};
    use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};
    use tokio::net::UnixListener;
    ...
    let daemon_server = tokio::spawn(async move {
        ...
        for _ in 0..2 {
            ...
            match command {
                DaemonCommand::Input { session_id, data } => {
                    assert_eq!(session_id, "run-input-current");
                    inputs.push(data);
                }
                other => panic!("expected Input command, got {other:?}"),
            }
    ...
    db.insert_stage_run(crate::db::NewStageRun {
        ...
        result: None,
        feedback: None,
        session_id: Some("run-input-current"),
        provider_session_id: None,
        cwd: Some("/tmp/task-input"),
        resumed_from_run_id: None,
    })
    .unwrap();
    drop(db);

    let response = super::router(Arc::new(super::AppState::new(config)))
        .oneshot(
            Request::post("/v1/tasks/task-input/input")
                .header("content-type", "application/json")
                .body(Body::from(
                    serde_json::json!({ "input": "continue" }).to_string(),
                ))
                .unwrap(),
        )
        .await
