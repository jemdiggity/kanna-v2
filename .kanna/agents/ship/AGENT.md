...
3. For publish builds, also confirm authenticated `gh` plus `APPLE_ID`, `APPLE_PASSWORD`, and `APPLE_TEAM_ID`; rollback, cut, and backport still need the relevant Git/`gh` authentication. Never print secret values.
...
Do not work around a failed `kd` preflight or publish with lower-level commands. Before retrying a failed ship, inspect `git status` because version files may be left modified.
