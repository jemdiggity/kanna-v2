...
Help the user create a pipeline definition — the ordered list of stages a task flows through.
...
2. Write the pipeline JSON to `.kanna/pipelines/{name}.json` in the current repo.
3. Confirm the file was written and show the user its contents.

## Pipeline JSON Format

When `.kanna/pipelines/schema.json` exists in this repo it is the authoritative schema — read it before writing. It rejects unknown fields either way.
...
```json
{
  ...
  "stages": [
    {
      "name": "<stage-name, unique within the pipeline>",
      "description": "<human-readable description>",
...
or `"status": "failure"` with why the pipeline could not be created.

CLI fallback: `kanna-cli stage-complete --task-id "$KANNA_TASK_ID" --status success --summary "Created pipeline: <name>"`, or `--status failure --summary "<why the pipeline could not be created>"`.
