...
Mobile native identity is keyed by `KANNA_APP_ENV` from `apps/mobile/src/mobileEnvironments.json`. The iOS bundle ids are `build.kanna.app.dev` for dev, `build.kanna.app.staging` for staging, and `build.kanna.app` for production; display names are `Kanna Dev`, `Kanna Staging`, and `Kanna`. For local iOS projects that already have `apps/mobile/ios/`, `./kd mobile run --device` writes `apps/mobile/ios/KannaNativeIdentity.xcconfig`, patches the KannaMobile app target build settings to use `$(KANNA_BUNDLE_ID)` and `$(KANNA_DISPLAY_NAME)`, and passes those values through the build environment before invoking `expo run:ios`. This applies the correct app target bundle id without `expo prebuild --clean`; test targets such as WebDriverAgentRunner keep their own bundle ids. When `ios/` is absent and Expo generates native sources, `apps/mobile/plugins/withKannaNativeIdentity.js` applies the same identity during prebuild.
