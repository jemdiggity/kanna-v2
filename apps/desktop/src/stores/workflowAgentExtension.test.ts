...
function makeApi() {
  ...
  return createWorkflowApi(context);
}
...
function remoteWorkflow(stageName: string) {
  return {
    name: "qa",
    stages: [{
      name: stageName,
...
describe("revisioned server definitions", () => {
  ...
  it("reuses a workflow object for the same revision and replaces it when origin advances", async () => {
    const fetchRepoWorkflowDefinition = vi.fn()
      .mockResolvedValueOnce({ revision: "rev-1", definition: remoteWorkflow("review") })
      .mockResolvedValueOnce({ revision: "rev-1", definition: remoteWorkflow("ignored") })
      .mockResolvedValueOnce({ revision: "rev-2", definition: remoteWorkflow("pr") });
    updateDesktopServerClientHandlersForTests({ fetchRepoWorkflowDefinition });

    const api = makeApi();
    const first = await api.loadWorkflow("repo-1", "qa");
    const same = await api.loadWorkflow("repo-1", "qa");
    const changed = await api.loadWorkflow("repo-1", "qa");

    expect(fetchRepoWorkflowDefinition).toHaveBeenCalledTimes(3);
    expect(fetchRepoWorkflowDefinition).toHaveBeenCalledWith("repo-1", "qa");
    expect(same).toBe(first);
    expect(changed).not.toBe(first);
  ...
  it("propagates server definition errors without falling back to local resources", async () => {
    ...
    const fetchRepoWorkflowDefinition = vi.fn(async () => {
      throw new Error("invalid remote workflow at origin/main");
    });
    updateDesktopServerClientHandlersForTests({
    ...
    await expect(api.loadWorkflow("repo-1", "qa")).rejects.toThrow("invalid remote workflow");
    expect(invokeMock).not.toHaveBeenCalled();
  });
});
