...
describe("Kanna repository cache defaults", () => {
  ...
  it("installs the pinned compiler cache after environment sync in every Kanna-managed worktree", () => {
    expect(config.setup).toEqual(["pnpm install", "./kd env sync", "./kd rust-cache install"]);
  });
