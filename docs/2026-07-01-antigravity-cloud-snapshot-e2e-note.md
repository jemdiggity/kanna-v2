...
Antigravity provider propagation crosses the local task model, singleton `kanna-server` publisher, relay Admin reconciliation, Firestore-backed cloud task index, and the remote task UI model. Renderer windows are read-only task-index consumers.
...
- `crates/kanna-server/src/cloud_task_publisher.rs` verifies provider values are preserved by the server-owned snapshot mapper.
- Relay publication tests verify the authenticated server snapshot is reconciled through Firebase Admin.
