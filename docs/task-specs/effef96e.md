...
Implementation decision (2026-08-27): tear-offs reuse the existing Tauri app-window creation lifecycle rather than introducing a separate window implementation. No matching durable task input records this intermediate decision as an owner clarification.

Implementation scope decision (2026-08-27): torn-off surfaces participate in the same persisted window workspace lifecycle as ordinary Kanna windows and may restore after restart, replacing the initial no-restart-persistence restriction. No matching durable task input records the intermediate discussion that led to this decision.

Delivered revision directive (durable task input 353, delivered 2026-08-29, verbatim): “I tried it out. Let's make the modal -> new window transition happen when the drag starts and just make it a normal window. For example, if you drag the tree explorer and make a new window, you can then exit out of the tree explorer and it's a normal window.” This replaces release-to-create behavior: crossing the drag threshold creates the native window immediately and hands the continuing drag to it. The detached surface initially fills that window; dismissing the surface converts the same OS window into an ordinary Kanna workspace rather than closing it. Closing the OS window itself remains an independent window close.

Implementation refinement (2026-08-29): the special standalone-surface bootstrap and later promotion/reload were replaced so the new webview runs the ordinary Kanna app from its first render, with the transferred modal opened through its normal modal layer in maximized state. Closing that modal reveals the already-running ordinary workspace beneath it without reloading the window. This realizes the normal-window behavior required by durable task input 353; no separate durable input records the intermediate refinement as an owner clarification.

Implementation polish (2026-08-29): the automatic startup shortcuts prompt is suppressed when a window boots with a transferred modal, so the maximized explorer or diff remains the topmost initial surface. The normal explicit shortcut-menu command remains available. No matching durable task input records this intermediate polish as an owner refinement.

Implementation polish (2026-08-29): tear-off header/chrome retains the normal arrow cursor; the drag gesture and threshold remain unchanged. No matching durable task input records this intermediate polish as an owner directive.
...
Owner on-machine verification relay (durable task input 376, delivered 2026-08-30): “I tested it and it works ok”. The relay reports that the owner exercised the modal tear-off interaction in the dev desktop and approved it with no further polish requested. This satisfies the required human on-device testing gate for the UI-feel change and clears the task to proceed to review.

Reviewer revision round 2 (2026-08-30): fix tear-offs created after source-window navigation so the new workspace row copies the live repo/task selection rather than immutable startup bootstrap state. Restored explorer and diff surfaces must keep the transferred repo, worktree, task metadata, view state, and explorer file previews coherent with what was visible at drag time. Add focused workspace/modal tests and extend the multi-window E2E to navigate after startup and distinguish the new selection from the startup selection for both surfaces.

Verified 2026-08-30 for revision round 2: focused workspace/modal/App tests and both desktop TypeScript checks pass, and `./kd test all` reports canonical local verification clean. The debug multi-window WebDriver E2E passes with a source window bootstrapped on one repo/task and navigated to another before each tear-off; the secondary explorer proves the new repo/task selection and opens the new repo's unique file preview, while the secondary diff proves the same new repo/task context and unique diff content.
