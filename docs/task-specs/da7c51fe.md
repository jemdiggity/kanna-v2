...
The defect's cause is evidenced, current tokens replace obsolete registrations, permanent bad-token failures remove the token and surface a distinct no-valid-token reason, and `cargo test -p kanna-server` plus the relevant mobile/relay suites pass.
