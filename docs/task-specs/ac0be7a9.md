...
- Mobile automatically recovers on selection, shows "restarting session…" while pending, shows server failure reasons, and invokes the shared resume API over LAN and relay.
...
- Mobile TypeScript passed. Six focused suites passed with 338 tests, covering
  the controller, LAN/relay transports, and rendered restart states; the final
  controller-only rerun passed all 209 tests.
