...
The message is `{ type: "task_snapshot_publish", id, snapshot }`. `snapshot` contains `schemaVersion: 1`, the desktop display name, and the complete current set of open task documents. Task documents preserve the existing fields: owner/local identities, title and prompt snippets, stage/activity/status, repository identity and remote URL/hash, branch/base/PR metadata, agent provider/type, transfer defaults, blockers, and lifecycle timestamps. `promptSnippet` is a bounded copy of the task's original user prompt; `waitingPromptSnippet` is a separately bounded copy of SQLite `last_output_preview`, representing the latest assistant output or question that the mobile row may show. The mobile presentation must not substitute the original prompt for this waiting text.
...
- `pnpm --dir apps/desktop test:e2e:cloud-mobile-index` starts `kanna-server`, relay, and fresh Firebase emulators, changes SQLite `last_output_preview` and activity, and reads the resulting `waitingPromptSnippet` and `working` state through `apps/mobile/src/lib/firebase/taskIndex.ts`.
