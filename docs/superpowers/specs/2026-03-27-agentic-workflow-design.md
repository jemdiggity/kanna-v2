...
Replace Kanna's hardcoded tag-based workflow with a generic, user-definable agentic workflow system. Agents become first-class file-based definitions, workflows become JSON configuration, and a workflow engine orchestrates task progression through stages.

## Terminology

| Term | Definition |
|------|-----------|
| **Agentic Workflow** | A JSON file defining an ordered sequence of stages with transition rules. The overall process that orchestrates agent work. |
| **Stage** | A step in an agentic workflow with an agent, an environment, and a transition type (manual/auto). |
...
| **Task** | A unit of work (`pipeline_item`) that flows through an agentic workflow. |
| **Stage-complete signal** | A CLI call made by an agent to signal it has finished its stage work. |

Key separation from the current model: **stage != tag**. Stages represent where a task is in the workflow. Tags (deferred to future work) are user-defined metadata labels for categorization.

## Workflow Model

A workflow is an ordered list of stages that a task goes through. Each stage has an agent and an environment. Every task runs in its own worktree. Workflows are independent — they do not chain into each other.

Cross-workflow coordination (e.g., a merge agent that processes PRs from multiple tasks) is handled by the agent itself, not by workflow orchestration. The merge agent's AGENT.md instructs it to scan for PRs; it discovers its own work rather than receiving tasks from another workflow.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | yes | Stage identifier, unique within workflow |
...
`metadata` is free-form. Agents can pass data to the next stage (e.g., `pr_url`, `test_results`). The workflow engine makes this available to the next stage's agent via environment variables or prompt injection.
...
- Runs workflow engine logic (teardown, blocker check, advance)
...
|--------|--------|------|-------------|
| Add | `workflow` | TEXT NOT NULL DEFAULT 'default' | Workflow name |
...
1. Add `workflow` column with default `'default'`
...
No new tables. Workflows and agents are files, not DB rows. The DB only tracks task state.

### Teardown clarification

Two distinct teardown concepts exist:
- **Task-close teardown:** repo-level `.kanna/config.json` `teardown` scripts that run when a task is closed (Cmd+Delete). This stays as-is — it's about cleaning up the worktree, not about workflow stages.
- **Stage environment teardown:** scripts defined in a workflow environment that run when a task exits a stage. This is new.

These are independent. Task-close teardown runs regardless of workflow state. Stage environment teardown runs on stage transitions.

## Workflow Engine

The workflow engine is a function in the Kanna store. It runs when:
...
1. Read task's current workflow and stage from DB
2. Load workflow JSON definition from disk. If file is missing: freeze task in current stage, toast "Workflow definition not found", abort.
...
6. Find next stage in the workflow
...
- Workflow definition missing: task freezes in current stage, toast notification
...
New: pinned -> grouped by stage in workflow order -> blocked

Stage sections are dynamic, derived from each task's workflow definition. If a repo has tasks in multiple workflows, stages from all active workflows are shown, ordered by workflow then stage order. Empty stages are hidden.
...
- **Prompt** (existing)
- **Workflow** selector (dropdown, defaults to repo's default workflow from `.kanna/config.json`)

The first stage's agent is used automatically. The workflow defines the agent, not the user. The selected workflow name is stored in the `workflow` column of the new `pipeline_item` row on insert.
...
Two built-in agents for creating new agents and workflows:

- **agent-factory**: user describes a role, agent writes `.kanna/agents/{name}/AGENT.md`
- **workflow-factory**: user describes a flow, agent writes `.kanna/workflows/{name}.json`

Factory agents are regular task agents. They go through the full workflow (worktree, PR, merge) because their output is code changes to the repo. They use the repo's configured default workflow like any other task.

Accessible via command palette as shortcuts: "Create Agent" and "Create Workflow" pre-configure a task with the factory agent selected.

## What Gets Removed

- `TagBadges.vue` component
- Hardcoded `tagColors` and `tagLabels` maps
- `SYSTEM_TAGS` constant in `packages/core/src/workflow/types.ts`
...
The built-in default workflow replicates today's hardcoded flow with all manual transitions:
...
The merge/shipping agent is a separate task the user creates independently. Its AGENT.md instructs it to scan for PRs and merge them. It is not part of this workflow — it runs its own workflow (which could be a single-stage workflow with just an "in progress" stage).

Users can copy and modify the default workflow to create their own with custom stages, agents, environments, and auto transitions.
