...
When an optional LAN read reaches its 1,000 ms deadline (or encounters the same unresolved shared read already in flight), the hybrid routing layer will expire synchronous LAN routability before returning its preserved task projection. It will then publish a route-identity change. This makes the optional-read deadline the single boundary at which stream routing stops treating the old URL as usable; the longer Bonjour probe may still finish for inventory and diagnostics, but cannot keep task traffic pinned to stale LAN state.
