...
Replace the 5-stage workflow (`queued`, `in_progress`, `needs_review`, `merged`, `closed`) with a 3-stage workflow (`in_progress`, `pr`, `done`). Add a PR agent stage that spawns a haiku-model Claude session to rename the branch and create a GitHub PR via `gh`.
...
**Session ID**: The PR agent reuses the same session ID as the coding agent (the workflow item ID). The coding agent's session must be killed first. The terminal UI naturally shows the new session since it's attached to the same ID.

**Implementation**: New `startPrAgent(itemId)` function in `useWorkflow.ts`. Reuses `spawnPtySession()` with different model and prompt args.

## Code Changes

### Modify

- **`packages/core/src/workflow/types.ts`** — Replace `Stage` type with `"in_progress" | "pr" | "done"`. Simplify `VALID_TRANSITIONS` to 3 rules. Remove label references.
- **`packages/core/src/workflow/transitions.ts`** — No logic changes, just works with new types.
- **`packages/core/src/workflow/transitions.test.ts`** — Update tests for new stages.
- **`apps/desktop/src/composables/useWorkflow.ts`** — Add `startPrAgent()`. Modify `spawnPtySession()` to accept model override.
