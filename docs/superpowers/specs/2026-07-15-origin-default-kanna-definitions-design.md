...
Each orchestration operation resolves that remote-tracking ref to one immutable commit and reads all relevant `.kanna` resources from the commit. This prevents a stale, dirty, or otherwise neglected local default branch from supplying a pipeline, agent, or configuration value that disagrees with the code available on the remote default branch.
...
- `.kanna/config.json`
- `.kanna/pipelines/<name>.json`
- `.kanna/agents/<role>/AGENT.md`
- `.kanna/agents/<role>/EXTEND.md`

The desktop also reads the local pipeline directory, config file, and agent files when it prepares task-creation choices or resolves agent metadata. The repository path is normally the main checkout, but Kanna does not keep that checkout's local default branch synchronized. As a result:

- task code can start from a freshly fetched `origin/main` while its pipeline and prompt came from stale local `main`;
- uncommitted local `.kanna` edits can affect task execution unexpectedly;
- the desktop can display or submit a local pipeline choice that differs from what the server should execute;
- config fields can be mixed across sources even if agents and pipelines move to the remote ref independently.
...
3. Read every field in `.kanna/config.json`, pipeline definitions, agent definitions, and agent extensions from that same commit.
...
6. Keep bundled pipelines and agents as the fallback for repositories without the corresponding remote file.
...
- Replacing the durable pipeline JSON snapshot already stored on a task.
- Loading uncommitted `.kanna` changes from the imported checkout or a task worktree.
- Changing the schema or behavior of pipeline, agent, or repo-config fields.
...
The existing repo-config, pipeline, agent, and extension parsers remain the validation source of truth. Their input changes from a filesystem path to a `RepoDefinitionSnapshot`.
...
| Named pipeline | Parse the remote file | Try the matching bundled pipeline, then return the existing not-found error |
...
Top-level task operations create one snapshot and pass it through provider resolution, prompt construction, port allocation, setup/teardown resolution, and agent/pipeline loading. Helpers must not independently re-resolve the moving remote ref during the same operation.

### Durable tasks

Task creation continues to serialize the normalized pipeline definition into `pipeline_def`. Later stage transitions continue to use that stored pipeline snapshot, preserving the task's pinned stage topology and policies.
...
The server is the source of truth for remote-backed Kanna definitions. The desktop will stop reading `.kanna/config.json`, `.kanna/pipelines`, `AGENT.md`, and `EXTEND.md` directly for orchestration-related UI.

Repository definition endpoints will be:

- `GET /v1/repos/{repo_id}/kanna-definitions` returns `{ revision, refName, config, defaultPipeline, pipelines }`;
- `GET /v1/repos/{repo_id}/kanna-definitions/pipelines/{pipeline_name}` returns `{ revision, definition }`;
- `GET /v1/repos/{repo_id}/kanna-definitions/agents/{agent_selector}` returns `{ revision, definition }`.

`revision` is the resolved commit id or `null` for a bundled-only source. `refName` is the attempted remote-tracking ref such as `origin/main`. `config` is the complete parsed remote repo config, or `{}` for a bundled-only source. `pipelines` is the deduplicated, sorted union of remote pipeline names and bundled pipeline names; a remote file overrides a bundled definition with the same name. Path parameters are percent-encoded by the client and validated as definition names by the server.

Responses include the resolved commit id so desktop pipeline and agent caches can be keyed or invalidated by revision rather than only by repository path and definition name.

The new-task modal uses the manifest for pipeline choices and default selection. Desktop stage-order caching, task-session recovery, and worktree shell environment construction use the manifest's complete config instead of reading a checkout. Setup-agent and merge-agent preparation use the server-resolved agent endpoint. Optimistic stage projections use the server-resolved pipeline endpoint. Actual task execution remains authoritative if the remote changes between displaying the modal and submitting the task.
...
Task/config request
  ...
  -> read config + pipeline + agent resources from that commit
...
- A failed fetch with no cached ref produces a bundled-only source. Default configuration and bundled agents/pipelines remain usable; a requested custom definition returns a clear not-found error.
- An invalid remote config, pipeline, agent, or extension reports the remote ref/commit and resource path in the error.
...
- local config, pipeline, agent, and extension contain sentinel values that must never appear;
- remote config exercises pipeline, flavors, vars, ports, setup, teardown, and workspace PATH fields;
...
- the selected/default pipeline comes from remote config;
...
- the stored pipeline snapshot matches the remote pipeline;
...
- repository definition endpoints return remote-backed manifest, pipeline, and agent responses with a commit id;
- the new-task modal uses the remote pipeline list and default;
- stage ordering, recovered task sessions, and task worktree shells use the remote config returned by the manifest;
- agent and pipeline caches change when the returned commit id changes;
...
- focused desktop Vitest suites for task creation and pipeline/agent loading;
- `./kd test rust`;
...
- `apps/desktop/src/stores/pipeline.ts` and related tests.
