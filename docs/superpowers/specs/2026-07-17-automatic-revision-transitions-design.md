...
Keep the first implementation handoff in the default pipeline human-gated, then let implementer/reviewer revision loops transition automatically. Preserve pipeline customization so any stage can keep revisions manual.
...
- When `revision_transition` is absent, it falls back to `transition`. Existing custom and pinned pipeline definitions therefore retain their current behavior.
...
Existing stage-run rows created before the new field are handled by falling back to the pinned stage policy during completion. No task's pinned pipeline definition is rewritten.
...
1. A new default-pipeline task starts `in progress` with effective transition `manual`.
...
The TypeScript and Rust pipeline loaders validate `revision_transition` with the same accepted values and error style as `transition`. The JSON schema and pipeline-factory documentation expose the new field. Normalized and pinned pipeline JSON retains it.
...
Invalid `revision_transition` values reject the pipeline definition rather than silently reverting to another behavior.
...
- Existing pipeline definitions without `revision_transition` retaining their current semantics.
...
- Rewriting existing tasks' pinned pipeline definitions.
- Adding a global preference that overrides per-pipeline policy.
