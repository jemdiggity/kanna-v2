...
**Goal:** Replace hardcoded tag-based workflow with user-definable agentic workflows — agents as `.md` files, workflows as `.json` files, and a workflow engine that advances tasks through stages.

**Architecture:** File-first approach. Agent definitions (AGENT.md) and workflow definitions (JSON) live on disk. The DB stores task state (which workflow, which stage). A new `kanna-cli` sidecar lets agents signal stage completion. The Tauri app listens on a Unix socket for these signals and runs the workflow engine to advance tasks.

**Tech Stack:** TypeScript (Vue 3, Pinia), Rust (Tauri, tokio), SQLite, YAML frontmatter parsing

**Spec:** `docs/superpowers/specs/2026-03-27-agentic-workflow-design.md`
...
| `.kanna/agents/workflow-factory/AGENT.md` | Factory agent for creating new workflows |
| `.kanna/workflows/default.json` | Built-in default workflow definition |
| `packages/core/src/workflow/workflow-types.ts` | Workflow & stage TypeScript interfaces |
| `packages/core/src/workflow/workflow-loader.ts` | Load & validate workflow JSON from disk |
| `packages/core/src/workflow/agent-loader.ts` | Load & validate AGENT.md from disk |
| `packages/core/src/workflow/prompt-builder.ts` | Build stage prompt with variable substitution |
| `packages/core/src/workflow/workflow-loader.test.ts` | Tests for workflow loading |
| `packages/core/src/workflow/agent-loader.test.ts` | Tests for agent loading |
| `packages/core/src/workflow/prompt-builder.test.ts` | Tests for prompt building |
...
| File | Changes |
|------|---------|
| `packages/core/src/workflow/types.ts` | Remove `SYSTEM_TAGS`, `parseTags`, `hasTag`. Add stage helpers. |
| `packages/core/src/config/repo-config.ts` | Add `workflow` field to `RepoConfig` |
| `packages/db/src/schema.ts` | Add `workflow`, `stage_result` to `PipelineItem`. Remove `tags` usage. |
| `packages/db/src/queries.ts` | Replace tag queries with stage queries. Add `updatePipelineItemStage()`. |
| `apps/desktop/src/stores/db.ts` | Add migration for `workflow`, `stage_result` columns. Tag-to-stage data migration. |
| `apps/desktop/src/stores/kanna.ts` | Replace tag-based logic with workflow engine. Remove `startPrAgent`/`startMergeAgent`. Add `advanceStage()`, `rerunStage()`. |
...
| `apps/desktop/src/components/NewTaskModal.vue` | Add workflow selector dropdown. |
| `apps/desktop/src/components/CommandPaletteModal.vue` | Add "Create Agent" and "Create Workflow" entries. |
| `apps/desktop/src-tauri/src/lib.rs` | Add `kanna.sock` Unix socket listener. |
| `apps/desktop/src-tauri/src/commands/mod.rs` | Register new workflow commands if needed. |
...
## Task 0: Built-in agent and workflow definition files
...
- Create: `.kanna/agents/workflow-factory/AGENT.md`
- Create: `.kanna/workflows/default.json`
...
- [ ] **Step 5: Write workflow-factory AGENT.md**

Agent that helps users create new workflow definitions. Its instructions include the workflow JSON schema spec.

- [ ] **Step 6: Write default.json workflow**
...
```bash
git add .kanna/agents/ .kanna/workflows/
git commit -m "feat: add built-in agent and workflow definition files"
```

---

## Task 1: Workflow and agent TypeScript types

**Files:**
- Create: `packages/core/src/workflow/workflow-types.ts`
- Modify: `packages/core/src/workflow/types.ts`

- [ ] **Step 1: Write workflow-types.ts with interfaces**

```typescript
// packages/core/src/workflow/workflow-types.ts
...
```bash
git add packages/core/src/workflow/
git commit -m "feat: add workflow and agent TypeScript type definitions"
```

---

## Task 2: Workflow loader (load + validate workflow JSON)

**Files:**
- Create: `packages/core/src/workflow/workflow-loader.ts`
- Create: `packages/core/src/workflow/workflow-loader.test.ts`

- [ ] **Step 1: Write failing tests for workflow-loader**
...
Run: `cd packages/core && bun test workflow-loader`
Expected: FAIL — module not found.

- [ ] **Step 3: Implement workflow-loader.ts**

Functions:
- `parseWorkflowJson(raw: string): WorkflowDefinition` — parse and validate
- `validateWorkflow(def: WorkflowDefinition): string[]` — return list of validation errors
...
```bash
git add packages/core/src/workflow/workflow-loader*
git commit -m "feat: add workflow JSON loader with validation"
```
...
**Files:**
- Create: `packages/core/src/workflow/agent-loader.ts`
- Create: `packages/core/src/workflow/agent-loader.test.ts`
...
```bash
git add packages/core/src/workflow/agent-loader*
...
**Files:**
- Create: `packages/core/src/workflow/prompt-builder.ts`
- Create: `packages/core/src/workflow/prompt-builder.test.ts`
...
```bash
git add packages/core/src/workflow/prompt-builder*
git commit -m "feat: add prompt builder with variable substitution"
```

---

## Task 5: Scanner for agents and workflows per-repo

**Files:**
- Create: `packages/core/src/workflow/scanner.ts`
- Create: `packages/core/src/workflow/scanner.test.ts`

- [ ] **Step 1: Write failing tests**

Test: scan `.kanna/agents/*/AGENT.md` and `.kanna/workflows/*.json` from a repo path. Mock filesystem reads via the same `listDir`/`readTextFile` pattern used in `custom-tasks-scanner.ts`.
...
- `scanWorkflows(repoPath: string, readFile: ReadFn, listDir: ListDirFn): Promise<WorkflowDefinition[]>`
...
```bash
git add packages/core/src/workflow/scanner*
git commit -m "feat: add agent and workflow scanner"
```
...
Add fields:
```typescript
workflow: string;     // workflow name (e.g., "default")
...
- [ ] **Step 3: Update insertPipelineItem to accept workflow and stage**

Modify the INSERT statement to include `workflow` and ensure `stage` defaults to the first stage name.

- [ ] **Step 4: Update existing tests in queries.test.ts**

Add tests for new stage-related queries. Update mock data to include `workflow` and `stage` fields.
...
git commit -m "feat: add workflow and stage columns to DB schema and queries"
```
...
```typescript
// Add workflow column
await safeAlterTable(db, "pipeline_item", "workflow", "TEXT NOT NULL DEFAULT 'default'");
...
git commit -m "feat: add workflow/stage DB migration from tags"
```

---

## Task 8: Update repo config to support workflow field

**Files:**
- Modify: `packages/core/src/config/repo-config.ts`

- [ ] **Step 1: Add `workflow` to RepoConfig interface**
...
export interface RepoConfig {
  ...
  workflow?: string; // default workflow name
}
```

- [ ] **Step 2: Update parseRepoConfig() to parse the new field**

Validate that `workflow` is a string if present.
...
git commit -m "feat: add workflow field to RepoConfig"
```
...
Add a `spawn_workflow_listener()` function that:
...
4. Emits a Tauri event `workflow_stage_complete` with the task_id
5. Disconnects

- [ ] **Step 2: Call spawn_workflow_listener() from the app setup**

Add it alongside `spawn_event_bridge()` in the Tauri builder setup.

- [ ] **Step 3: Pass workflow env vars to agent spawn**
...
## Task 11: Workflow engine in store

**Files:**
- Modify: `apps/desktop/src/stores/kanna.ts`

This is the largest task. Replace the tag-based state machine with the workflow engine.

- [ ] **Step 1: Add workflow loading to the store**

Add a `loadWorkflow(repoPath: string, workflowName: string)` function that uses the scanner/loader from `@kanna/core` to read the workflow JSON. Cache loaded workflows in a reactive map.

- [ ] **Step 2: Update createItem() to use workflow**

Modify `createItem()` to:
- Accept `workflowName` parameter (default: from repo config or `"default"`)
- Load the workflow definition
- Set `workflow` and `stage` (first stage name) on the new DB row
...
1. Read task's workflow and stage from DB
...
- [ ] **Step 5: Listen for workflow_stage_complete Tauri event**

In the store's `init()` function, listen for the `workflow_stage_complete` event. When received:
...
- Load the workflow definition for each item
- Group by stage in workflow stage order
...
git commit -m "feat: implement workflow engine with advanceStage and rerunStage"
```
...
Remove `sortedMerge()`, `sortedPR()`, `sortedActive()`. Replace with a single computed that groups items by their `stage` field, ordered by the workflow's stage order.

```typescript
const groupedByStage = computed(() => {
  // Group non-pinned, non-blocked items by stage
  // Order stages according to workflow definition
...
## Task 15: NewTaskModal workflow selector

**Files:**
- Modify: `apps/desktop/src/components/NewTaskModal.vue`

- [ ] **Step 1: Add workflow selector dropdown**

Add a `<select>` or custom dropdown that lists available workflows for the current repo (scanned from `.kanna/workflows/`). Default to the repo's configured workflow from `.kanna/config.json`.

- [ ] **Step 2: Pass selected workflow to submit event**

Change the `submit` event to emit `(prompt, workflowName)` instead of just `prompt`.

- [ ] **Step 3: Update App.vue handler**

Update the `handleNewTask` handler to pass `workflowName` through to `createItem()`.

- [ ] **Step 4: Commit**

```bash
git add apps/desktop/src/components/NewTaskModal.vue apps/desktop/src/App.vue
git commit -m "feat: add workflow selector to new task modal"
```
...
- [ ] **Step 1: Add "Create Agent" and "Create Workflow" dynamic commands**
...
},
{
  ...
  execute: () => {
    store.createItem(repoId, repoPath, "Help me create a new workflow", "pty", { customTask: workflowFactoryConfig })
  }
}
```
...
git commit -m "feat: add Create Agent and Create Workflow to command palette"
```
...
The shortcut description should show "Advance to {nextStageName}" based on the current task's workflow state.
...
## Task 21: Integration test — full workflow flow

**Files:**
- Create: `apps/desktop/tests/e2e/mock/workflow-flow.test.ts` (or add to existing)

- [ ] **Step 1: Write E2E test for basic workflow flow**

Test: create task with default workflow → verify it starts in "in progress" stage → manually advance → verify it moves to "pr" stage → verify sidebar grouping updates.
...
Expected: New workflow tests pass alongside existing tests.

- [ ] **Step 4: Commit**

```bash
git add apps/desktop/tests/
git commit -m "test: add E2E tests for workflow flow"
```
...
```
Task 0 (agent/workflow files) — no deps
Task 1 (types) — no deps
Task 2 (workflow loader) — depends on Task 1
...
Task 11 (workflow engine) — depends on Tasks 2, 3, 4, 5, 6, 7, 8, 10
