...
**Goal:** Replace the 5-stage workflow with a 3-stage workflow (`in_progress`, `pr`, `done`) and add a PR agent that spawns a haiku-model Claude PTY session to create GitHub PRs.

**Architecture:** Core workflow types simplified in `packages/core/`, DB migration in `App.vue`, new `startPrAgent()` in `useWorkflow.ts` that spawns a PTY session with `--model haiku`. Hook handler auto-transitions `pr` → `done`. Dead code (PR workflow, GitHub client, merge flow) deleted.

**Tech Stack:** Vue 3, TypeScript, SQLite, Tauri, Claude CLI (PTY via daemon)

**Spec:** `docs/superpowers/specs/2026-03-20-workflow-simplification-design.md`

---

### Task 1: Update workflow types and transitions

**Files:**
- Modify: `packages/core/src/workflow/types.ts`
- Modify: `packages/core/src/workflow/transitions.ts`
- Modify: `packages/core/src/workflow/transitions.test.ts`

- [ ] **Step 1: Rewrite the transition tests for 3 stages**

```typescript
// packages/core/src/workflow/transitions.test.ts
...
Run: `cd packages/core && bun test src/workflow/transitions.test.ts`
...
git commit -m "refactor(workflow): simplify to 3 stages (in_progress, pr, done)"
```
...
```typescript
  // Workflow simplification: map old stages to new
  try {
...
git commit -m "refactor(app): migrate stages, update GC/hooks for 3-stage workflow"
```
...
In `useWorkflow.ts`, update `spawnPtySession` signature to accept an optional `model` parameter:
...
Add after `spawnPtySession` in `useWorkflow.ts`:
...
- [ ] **Step 3: Export startPrAgent from useWorkflow**
...
```typescript
const { items, selectedItemId, loadItems, transition, createItem, spawnPtySession, startPrAgent, selectedItem, pinItem, unpinItem, reorderPinned } = useWorkflow(db);
```
...
```typescript
  { action: "merge",      label: "Merge PR",          group: "Workflow",   key: "m",                            meta: true,               display: "⌘M" },
```
...
```bash
git add apps/desktop/src/composables/useWorkflow.ts apps/desktop/src/App.vue apps/desktop/src/composables/useKeyboardShortcuts.ts
git commit -m "feat(workflow): add PR agent stage with haiku model PTY session"
```
...
git commit -m "refactor(ui): update components for 3-stage workflow"
```
...
This e2e test needs significant changes for the new workflow:
...
git commit -m "chore: update remaining stage references for workflow simplification"
```
