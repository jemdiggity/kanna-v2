...
`apps/mobile/src/appLifecycle.test.ts` covers the AppState/timer transition matrix, including an indefinitely inactive switcher peek, a background return inside the 20-second grace, and grace expiry. `apps/mobile/src/state/mobileController.test.ts` verifies that the retained terminal stays current while hidden instead of accumulating a foreground replay queue, a grace refresh preserves the attachment, and expiry uses the existing reconnect path. `tests/remote-e2e/src/lan-layer.e2e.test.ts` exercises a large daemon buffer through a real server and the mobile transport, asserts that the initial phone snapshot is bounded, then drives the lifecycle through a within-grace background → active transition and asserts that no second snapshot is delivered.
